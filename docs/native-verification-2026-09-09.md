...
| iOS | Simulator and unsigned device builds passed with Xcode 26.3 | Simulator package installed and launch command accepted; see the runtime/signing limitations below | Simulator build and source; physical devices require signing |
...
surfaces`. This build host has not yet provided a verified interactive iOS
render; installation acceptance is not a successful visual test. Treat the
simulator artifact as a developer preview and validate on your own supported
Xcode host before relying on it.
...
Public release archives include SHA-256 checksums. Credentials, signing keys,
provisioning profiles, browser profiles, SDKs and private logs are excluded.
