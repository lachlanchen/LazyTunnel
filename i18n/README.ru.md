...
[Руководство по публикации в Apple](../docs/apple-publication.md): процесс на основе опыта EchoMind, явный план выпуска и команда проверки готовности без изменения данных. Предварительная версия iOS ещё не загружена в TestFlight и не отправлена на App Review.


## Установка CLI через npm

Один небольшой пакет содержит команды клиента и управления ретранслятором Linux. Установка npm не запускает службы и не меняет регистрацию устройств, учётные данные или удалённые рабочие столы. Нативное графическое приложение загружается отдельно.
...
[Полное руководство по установке, регистрации и обновлению через npm](../docs/npm.md).

## Один ретранслятор, отдельные учётные записи

Ядро с лицензией MIT можно использовать для своих устройств или сервиса с независимыми учётными записями. CLI 0.3.0 добавляет вход по ключу или паролю, приглашения с закреплённым ключом сервера, а также регистрацию и отзыв устройств только их владельцем. У каждой учётной записи собственный список устройств и разрешения SSH. Существующие группы устройств сохраняются до явного обновления.

После получения приглашения от администратора сервера:
...
[Настройка учётных записей, изоляция, ограничения и миграция](../docs/accounts.md).
